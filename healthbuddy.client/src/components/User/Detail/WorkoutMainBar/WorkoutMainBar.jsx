...
const WorkoutMainBar = () => {
  ...
  const handleDetailClick = (detail) => {
    console.log(detail);
    navigate(`/detail/exercise/${detail.exerciseId}`);
  };

  const fetchScheduleDetail = async () => {
    try {
      const response = await api.get(
        "/api/Workout/GetWorkoutScheduleById/" + postId
      );
      const mappedDetail = {
        id: response.data.workoutScheduleId,
        title: response.data.workOutName,
        image: response.data.imgUrl,
        description: response.data.description,
        totalDays: response.data.totalDays,
        details: response.data.workoutDetails.map((detail) => ({
          exerciseId: detail.exercise.exerciseId,
          exerciseName: detail.exercise.exerciseName,
          detailImg: detail.exercise.imgUrl,
